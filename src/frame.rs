...
impl WebSocketFrame {
    ...
    fn serialize_header(hdr: &WebSocketFrameHeader) -> u16 {
        let b1 = ((hdr.fin as u8) << 7) | ((hdr.rsv1 as u8) << 6) | ((hdr.rsv2 as u8) << 5) |
                 ((hdr.rsv3 as u8) << 4) | ((hdr.opcode as u8) & 0x0F);
    ...
    fn read_length<R: Read>(payload_len: u8, input: &mut R) -> io::Result<usize> {
        return match payload_len {
            ...
            PAYLOAD_LEN_U16 => {
                input.read_u16::<BigEndian>().map(|v| v as usize).map_err(From::from)
            }
